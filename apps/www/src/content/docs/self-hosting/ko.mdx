...
4. secret을 입력하는 화면이 나오면 아래 세 값을 넣습니다.
   - `AUTH_ALLOWED_EMAILS` - 계정을 만들 수 있는 이메일 주소를 쉼표로 구분한 값입니다. 예: `you@example.com,family@example.com`
   - `BETTER_AUTH_SECRET`
   - `SYNC_TOKEN_SECRET`
...
![필수 secret 입력란이 보이는 Deploy to Cloudflare 폼](./cloudflare-deploy-secrets.png)
