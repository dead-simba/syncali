...
サーバーは単純な Node.js プロセス（`src/self-host.ts`）で、[tsx](https://tsx.is/) を使って実行します。Docker は便利な選択肢ですが必須ではありません。すでに VM（Incus/LXC コンテナなど）を使っていて、Docker を依存関係として追加したくない場合に適した方法です。
