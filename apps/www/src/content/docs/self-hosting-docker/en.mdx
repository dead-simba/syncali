...
The server is just a Node.js process (`src/self-host.ts`, run with [tsx](https://tsx.is/)) - Docker is a convenience, not a requirement. This is a reasonable choice if you're already running a VM (an Incus/LXC container, for example) and would rather not add Docker as a dependency.
