...
- A file whose name begins or ends with a space, such as `V2.2 Ground Floor `, stopped sync with "ENOENT: no such file or directory". Vault paths were being trimmed, which rewrote the name to one that does not exist. Names typed into settings are still trimmed; names read from your vault are now left alone.
